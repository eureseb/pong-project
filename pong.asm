...
DATA SEGMENT PARA 'DATA'
	...
	WINDOW_WIDTH DW 140h                 ; variable for window width
	WINDOW_HEIGHT DW 0C8h                ; variable for window height
	WINDOW_BOUNDS DW 6                   ; variable used to check collisions early
	
	TIME_AUX DB 0                        ; variable used when checking if the time has changed
	GAME_ACTIVE DB 1                     ; checks if game is active (if 1 then active, if 0 then inactive)
	EXITING_GAME DB 0
	WINNER_INDEX DB 0                    ; he index of the winner (1 -> player one, 2 -> player two)
	...
	TEXT_MAIN_MENU_SINGLEPLAYER DB 'START PLAYING - S KEY','$' ;text with the singleplayer message
...
;       Shows the menu title
		MOV AH,02h                       ;set  page number
		MOV DH,04h                       ;set row 
		MOV DL,04h						cursor position
		MOV BH,00h                       ;set ;set column
		INT 10h							 
